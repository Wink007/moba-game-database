...
def get_hero(hero_id):
    ...
    if hero:
        ...
        # Парсимо relation з JSON (для адмінки)
        if hero.get('relation') and isinstance(hero['relation'], str) and hero['relation'].strip():
            try:
                hero['relation'] = json.loads(hero['relation'])
            except:
                hero['relation'] = None
        elif not hero.get('relation'):
            hero['relation'] = None
